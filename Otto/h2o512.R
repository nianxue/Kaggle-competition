...
h2oCv512 <- arrange(h2oCv[[1]], rowIndex)
h2oCv512[,2:10] <- 0
h2oCv512[,2:10] <- (arrange(h2oCv[[1]], rowIndex)[,2:10] + 
                         arrange(h2oCv[[2]], rowIndex)[,2:10] +
                         arrange(h2oCv[[3]], rowIndex)[,2:10])/3

h2oCv512 <- h2oCv512 %>% select(starts_with("class"))
names(h2oCv512) <- paste0("h2o512","class",1:9)
...
h2oPred512 <- h2oPred[[1]]
h2oPred512[,2:10] <- 0
h2oPred512[,2:10] <- (h2oPred[[1]][,2:10] + h2oPred[[2]][,2:10] + h2oPred[[3]][,2:10])/3

h2oPred512 <- h2oPred512 %>% select(starts_with("class"))
names(h2oPred512) <- paste0("h2o512","class",1:9)