...
data <- data %>% group_by(Family_Hist_Comb) %>% mutate(Family_Hist_CombCnt = n()) %>% ungroup()
...
data <- data %>% group_by(Product_Info_2) %>% mutate(Product_Info_2Cnt = n())
data <- data %>% group_by(Product_Info_2_type) %>% mutate(Product_Info_2_typeCnt = n()) %>% ungroup()

data <- data %>% ungroup() %>% select(-Product_Info_2, -Product_Info_2_type, -Family_Hist_Comb)
...
split <- data$split
train <- data %>% select(-Id, -split, -Response) %>% filter(split == 1)
test <- data %>% select(-Id, -split, -Response) %>% filter(split == 0)

train$Response <- Response[split == 1,]
